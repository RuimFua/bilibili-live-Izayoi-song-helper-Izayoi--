...
// @version      4.9.0
// @description  强制边界锁定、右下角智能重置、歌单筛选、点歌指令分离。
// @author       User
...
(function () {
    ...
    // --- 2. 歌单数据 ---
    // ⚠️ 请把你完整的 songs.json 数组内容完整粘贴在这里！
    const SONG_DATA = [
    ...
    const REFERENCE_CALLS = ["开口跪！", "神仙音色", "吃CD长大的吧", "全体起立！", "前方高能", "注入灵魂", "太温柔了", "破防了", "88888888"];
    ...
    function injectUI() {
        ...
        document.getElementById('helper-reset').onclick = () => { container.style.top = 'auto'; container.style.left = 'auto'; container.style.right = '20px'; container.style.bottom = '80px'; container.style.width = '340px'; container.style.height = '500px'; };
        document.getElementById('helper-toggle-ref').onclick = () => { const p = document.getElementById('helper-ref-panel'); p.style.display = p.style.display === 'none' ? 'flex' : 'none'; };
        document.getElementById('helper-call-save').onclick = () => { const i = document.getElementById('helper-call-input'); if(i.value) { callWords.push(i.value); localStorage.setItem(STORAGE_KEY, JSON.stringify(callWords)); renderCallWords(); i.value=''; } };
